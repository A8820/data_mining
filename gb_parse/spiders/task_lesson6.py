...
import scrapy
import requests
from gb_parse.spiders.items_task_lesson6 import InstaTag, InstaPost


class InstagramSpider_(scrapy.Spider):
    name = 'instagram'
    ...
    def get_post_item(edges):
        for node in edges:
            InstagramSpider_.get_image_from_insta(node['node']['display_url'], node['node']['id'])
            yield InstaPost(
                date_parse=dt.datetime.utcnow(),
                data=node['node']
            )
